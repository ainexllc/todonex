...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      ...
      "bg-card px-4 py-3 text-sm font-medium backdrop-filter backdrop-blur-sm",
      "border-2 border-border",
      "placeholder:text-muted-foreground placeholder:font-normal",
      "transition-all duration-200",
      "hover:border-primary/30 hover:bg-hover-bg hover:-translate-y-0.5",
      "focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10",
      "focus:bg-card focus:shadow-lg focus:shadow-primary/5",
      "active:translate-y-0 active:shadow-sm",
      "disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-muted",
      "grok-select touch-friendly", // 44px minimum height
      className
    )}
    {...props}
  >
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[10rem] overflow-hidden rounded-2xl",
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-xl py-2.5 pl-10 pr-3 text-sm font-medium",
